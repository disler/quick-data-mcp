...
class EnhancedDatasetManager:
    ...
    def _generate_optimization_suggestions(self, df: pd.DataFrame) -> List[str]:
        ...
        if len(high_missing) > 0:
            suggestions.append(f"Consider dropping columns with >50% missing: {list(high_missing.index)}")
    ...
    def _calculate_completion_percentage(self, dataset_name: str) -> float:
        ...
        completed = len([a for a in expected_analyses if any(a in performed for performed in state.analyses_performed)])
    ...
    def _generate_next_analysis_recommendations(self, dataset_name: str) -> List[str]:
        ...
        if not state.correlations_calculated and len([c for c in schema.columns.values() if c.suggested_role == 'numerical']) >= 2:
        ...
        for col in numerical_cols:
            if col not in state.distributions_analyzed:
                recommendations.append(f"Analyze distribution of '{col}' with analyze_distributions()")
